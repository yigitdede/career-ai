@php
    ...
    $notifications = is_array($workspaceNotifications ?? null) ? $workspaceNotifications : [];
@endphp

<header class="workspace-header flex items-center justify-between gap-4 bg-white/80 px-6 py-3 backdrop-blur dark:bg-slate-950/80 md:px-10">
    <div class="flex min-w-0 items-center gap-2">
        <button type="button" @click="sidebarOpen = !sidebarOpen"
            ...
            aria-controls="{{ $workspaceSidebarId }}"
            :aria-label="@js($workspaceMenuLabel)">
            <i data-lucide="menu" class="h-5 w-5" aria-hidden="true"></i>
        </button>
        <p class="truncate text-sm font-medium text-slate-700 dark:text-slate-200 md:hidden">{{ $workspaceBrand }}</p>
    </div>
    ...
    <div class="ml-auto flex items-center gap-1 sm:gap-2">
        <div class="hidden items-center gap-3 sm:flex"
            ...
            <div class="flex items-center gap-2 rounded-lg px-2 py-1 text-xs text-slate-500 dark:text-slate-400">
                <span class="inline-block h-2 w-2 rounded-full {{ ($workspaceApiHealth['ok'] ?? false) ? 'workspace-accent-dot' : 'bg-red-500' }}"></span>
                {{ ($workspaceApiHealth['ok'] ?? false) ? __('panel.header.api_connected') : __('panel.header.api_pending') }}
            </div>
            ...
            <div class="relative">
                <button type="button" @click="notifOpen = !notifOpen; langOpen = false"
                    ...
                    <span x-show="unreadCount() > 0"
                        class="workspace-accent-pill absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] font-bold"
                ...
                <div x-show="notifOpen" x-cloak @click.outside="notifOpen = false"
                    ...
                    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
                        ...
                        <button type="button" @click="markAllRead()" class="workspace-accent-link">{{ __('panel.header.mark_read') }}</button>
                    </div>
                        ...
                        <template x-for="item in notifications" :key="item.id">
                            <li class="border-b border-slate-100 px-4 py-3 last:border-0 dark:border-slate-800">
                                <p class="flex items-start gap-2 text-sm font-medium text-slate-800 dark:text-slate-100">
                                    <span x-show="item.unread" class="workspace-accent-dot mt-1.5 h-2 w-2 shrink-0 rounded-full"></span>
                ...
                <div x-show="langOpen" x-cloak @click.outside="langOpen = false"
                    ...
                    @foreach (['tr' => 'Türkçe', 'en' => 'English'] as $locale => $label)
                        <a href="{{ route($workspaceLocaleRoute, $locale) }}"
                            ...
                            {{ $label }}
                            @if ($currentLocale === $locale)<i data-lucide="check" class="workspace-accent-check h-4 w-4" aria-hidden="true"></i>@endif
                        </a>
                    @endforeach
                </div>
            </div>
