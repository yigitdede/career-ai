...
@php
    ...
    $canWrite = $isSuperAdmin || in_array('career_data.write', $adminPermissions, true);
    $canDelete = $isSuperAdmin || in_array('career_data.delete', $adminPermissions, true);
@endphp
                ...
                @forelse ($records['roles'] as $role)
                    <article class="p-5">
                        <div class="grid gap-3 md:grid-cols-[1fr_8rem_9rem_auto] md:items-center">
                            ...
                            @if ($canDelete)<form method="POST" action="{{ route('admin.career-data.destroy', ['resource' => 'roles', 'record' => $role['id']]) }}">@csrf @method('DELETE')<button class="{{ $deleteClass }}" type="submit">{{ __('career-data.delete') }}</button></form>@endif
                        </div>
                        <p class="panel-muted mt-3 text-sm">{{ implode(', ', $role['required_skills']) ?: '—' }}</p>
                        @if ($canWrite)<details class="mt-4 border-t border-slate-200 pt-3 dark:border-slate-800"><summary class="cursor-pointer text-sm font-medium admin-accent-text">{{ __('career-data.edit') }}</summary>
