...
<div class="mx-auto max-w-5xl" x-data="careerInterview({{ Js::from($interview) }}, @js(route('panel.interview.start')), @js(route('panel.interview.score', ['interviewId' => '__INTERVIEW_ID__'])), {{ Js::from(['failed' => __('panel.interview.failed')]) }})">
    ...
    <header class="mb-8 flex items-start justify-between gap-4">
        ...
        <!-- DEĞİŞTİRİLDİ: Tıklanınca direkt başlamak yerine Modal'ı açıyoruz -->
        <button type="button" class="rounded-xl bg-emerald-600 px-5 py-2 text-sm font-medium text-white disabled:opacity-60" :disabled="busy" @click="showLangModal = true" x-text="interview ? @js(__('panel.interview.restart')) : @js(__('panel.interview.start'))"></button>
    </header>
    ...
    <div x-show="showLangModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
        <div @click.away="showLangModal = false" class="w-full max-w-sm rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-800">
            ...
            <div class="flex flex-col gap-3">
                <button @click="start('tr')" class="flex w-full items-center justify-center gap-3 rounded-xl bg-slate-50 p-4 font-medium text-slate-700 transition-colors hover:bg-slate-100 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-700/50">
                    <span class="text-2xl">🇹🇷</span> Türkçe Pratik Yap
                </button>
                <button @click="start('en')" class="flex w-full items-center justify-center gap-3 rounded-xl bg-slate-50 p-4 font-medium text-slate-700 transition-colors hover:bg-slate-100 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-700/50">
                    <span class="text-2xl">🇬🇧</span> Practice in English
                </button>
            </div>
            
            <button @click="showLangModal = false" class="mt-5 w-full text-sm font-medium text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">İptal</button>
        </div>
    </div>
</div>
@endsection